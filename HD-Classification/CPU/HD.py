...
@timer
def encoding_rp_amx_(X_data, base_matrix, signed=False):
    A = torch.from_numpy(base_matrix).float()
    X = torch.tensor(X_data, dtype=torch.float32)

    enc_hvs = []
    with torch.amp.autocast('cpu', dtype=torch.bfloat16): # int8 doesn't work -> definitely look into this (needed for similarity search)
        # look into exposing some of the lower level functionality (oneMKL oneDNN -> they should have a python interface)
        for i in range(len(X)): # Eliminate this for loop by doing one big matrix multiplication
            hv = torch.matmul(A, X[i])
...
@timer 
def encoding_idlv_(X_data, lvl_hvs, id_hvs, D, bin_len, x_min, L=64):
    enc_hvs = []
    ...
    for i, sample in enumerate(X_data):
        ...
        bins = np.floor((sample - x_min) / bin_len)
        bins = np.clip(bins, 0, L-1).astype(int)

        lvl_vectors = lvl_hvs[bins]

        bound = lvl_vectors * id_hvs

        ones_vector = np.ones((1, bound.shape[0]))

        sum_vector = np.matmul(ones_vector, bound).flatten()

        enc_hvs.append(sum_vector)
...
def train(X_train, y_train, X_test, y_test, D=500, alg='rp', epoch=20, lr=1.0, L=64, amx=False):
